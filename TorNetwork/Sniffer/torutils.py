...
class TorController(object):
    ...
    def launch_tor_service(self, logfile='dev/null'):
        ...
        self.torrc_dict.update(
            {'DataDirectory': self.tmp_tor_data_dir, 'Log': ['INFO file %s' % logfile]})
        ...
        try:
            ...
            wl_log.critical(
                'Unable to connect to tor on port %s: %s' % (cm.SOCKS_PORT, exc))
            sys.exit(1)
        except:
...
class TorBrowserDriver(webdriver.Firefox, firefox.webdriver.RemoteWebDriver):
    ...
    def __init__(self, tbb_binary_path=None, tbb_profile_dir=None, tbb_logfile_path=None, tbb_version=cm.TBB_DEFAULT_VERSION, page_url='', capture_screen=True):
        ...
        self.profile.set_preference(
            'extensions.torlauncher.prompt_at_startup', 0)
        ...
        self.binary = self.get_tbb_binary(
            tbb_version=self.tbb_version, logfile=tbb_logfile_path)

        # Initialize capabilities
        self.capabilities = DesiredCapabilities.FIREFOX
        self.capabilities.update({'handlesAlerts': True, 'databaseEnabled': True,
                                  'javascriptEnabled': True, 'browserConnectionEnabled': True})

        try:
            super(TorBrowserDriver, self).__init__(firefox_profile=self.profile,
                                                   firefox_binary=self.binary, capabilities=self.capabilities)
            ...
            wl_log.error('Error connecting to Webdriver: %s' %
                         e, exc_info=True)
